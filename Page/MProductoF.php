...
$sql = $conexion->query("SELECT * FROM ProductoFierro WHERE IdFierro=$id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($imagen != "") {
        ...
        $sql_select_imagen = "SELECT Imagen FROM ProductoFierro WHERE IdFierro=?";
    ...
    $stmt = $conexion->prepare("UPDATE ProductoFierro SET NombreProducto=?, TipoMetal=?, TamaProducto=?, Modelo=?, Imagen=? WHERE IdFierro=?");
    $stmt->bind_param("sssssi", $nombreProducto, $tipoMetal, $tamanioProducto, $modelo, $imagen_contenido, $id);
    $stmt->execute();
    $stmt->close();

    // Redirigir después de la modificación
    header("Location: ProductosEspecificaciones-ProductoFierro.php");
    exit();
}
?>
...
<head>
    ...
    <meta charset="utf-8" />
        ...
        <title>Ecodata - Producto Fierro</title>
...
<body class="sb-nav-fixed">
    ...
    <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
            <!-- Navbar Brand-->
            <a class="navbar-brand ps-3" >Modificar Producto Fierro</a>
            <!-- Sidebar Toggle-->
                ...
                <div class="container-fluid px-4">
                    <?php while ($datos = $sql->fetch_object()) { ?>
                        <section class="form-register">
                            <h4>Modificar</h4>
                            <input type="hidden" name="IdFierro" value="<?= $id ?>" id="IdFierro" placeholder="Ingrese ID del producto fierro">
                            <input type="text" name="NombreProducto" value="<?= $datos->NombreProducto ?>" id="NombreProducto" placeholder="Ingrese nombre del producto">
                            <input type="text" name="TipoMetal" value="<?= $datos->TipoMetal ?>" id="TipoMetal" placeholder="Ingrese tipo de metal">
                            ...
                            <?php endif; ?>                         
                            <button type="submit" class="botons" name="btmModificarPF" value="Ok">Modificar</button>
                        </section>
                    <?php } ?>
                </div>
            </main>
